...
Return ONLY the JSON specification."""
        ...
        if "components" in spec:
            ...
            if not isinstance(components, list) or len(components) == 0:
                ...
                for i, comp in enumerate(components):
                    ...
                    if "type" not in comp:
                        errors.append(
                            f"Component '{comp.get('name', f'index {i}')}' missing 'type'"
                        )
    ...
    def _generate_readme(self, spec: dict) -> str:
        ...
        for c in components:
            comp_lines.append(
                f"| {c['name']} | {c.get('technology', '—')} | {c.get('purpose', '—')} | {c.get('type', '—')} |"
            )
        ...
        return {
            ...
            "api_endpoints": [
                ...
                },
                {
                    ...
                    "response_example": {"id": "uuid", "email": "string"},
                },
                {
                    "method": "GET",
                    "path": "/api/v1/jobs",
